...
class AnyTextModel(TorchModel):
    ...
    def init_model(self, font_path, ckpt_path, clip_path, translator_path, cfg_path, **kwargs):
        ...
        if self.use_translator == True:
            ...
            self.trans_pipe = pipeline(task=Tasks.translation, model=zh2en_path, device=device)
        else:
